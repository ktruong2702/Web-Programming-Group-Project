...
<head>
    ...
    <style>
        ...
        width: calc(100% / 3); /* Divide the table width by the number of columns */
        }
...
<body>
  ...
  <main id="home">
    ...
    <section class="table-container">
        ...
        <!-- this is when the user loggin only, if not the message "Please log in to view the user table" will display -->
        <?php if (isset($user)): ?> 
        ...
        // Retrieve the data from the "user" table
        $sql = "SELECT * FROM user";
        ...
        echo "<table>";
        echo "<tr><th>User ID</th><th>Username</th><th>Email</th><th>Setting</th></tr>";
        while ($row = mysqli_fetch_array($result)) {
        echo "<tr>";
        echo "<td>" . $row["id"] . "</td>";
        echo "<td>" . $row["name"] . "</td>";
        echo "<td>" . $row["email"] . "</td>";
        echo "<td><form method='post' action='delete_user.php'><input type='hidden' name='name' value='" . $row["name"] . "'><button type='submit'>Delete</button></form></td>";
        echo "</tr>";            
        }
        echo "</table>";
        } else {
